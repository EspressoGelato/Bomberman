...
class net2(nn.Module):
    def __init__(self, in_channels, num_actions):
        ...
        self.fc4 = nn.Linear(11 * 11 * 64, 50)
        self.fc5 = nn.Linear(50, num_actions)

    def forward(self, x):
        if type(x) is np.ndarray:
            x = torch.from_numpy(x) # the input x is state_feature: numpy.array(5,17,17)
        x = x.float()
...
class DQN:
    ...
    def initialize(self):
        return net2(in_channels=self.state_size, num_actions=self.action_size)
    ...
    def save_model(self, episode):
        torch.save(self.policy_net.state_dict(), './model_saved/DQN_{}.pt'.format(episode))
    ...
    def plot_loss(self, episode):
        ...
        plt.savefig('./loss_plot/mse_loss_episode{}.png'.format(episode))

    def plot_score(self,score, episode):
        steps = len(score)
        x = range(steps)
        plt.figure()
        plt.plot(x, score)
